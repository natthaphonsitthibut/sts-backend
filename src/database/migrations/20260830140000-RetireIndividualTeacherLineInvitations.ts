...
/**
 * Removes the retired per-teacher LINE verification link. Schools now issue one
 * shared verification link and the identity provider proves active membership.
 * Teacher messaging accounts are separate and remain intact.
 *
 * Existing invitation credentials are intentionally discarded. `down()`
 * restores the schema for rollback, but cannot restore expired credential rows.
 */
export class RetireIndividualTeacherLineInvitations20260830140000 implements MigrationInterface {
  name = 'RetireIndividualTeacherLineInvitations20260830140000';
