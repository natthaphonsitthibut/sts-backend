...
/**
 * Backfill legacy field-visit cases created before cases.student_uuid was
 * populated. The current schema already has cases.student_uuid -> student_term
 * via fk_cases_student_uuid; this migration only fills NULL values where the
 * existing case name + school context resolves to exactly one enrollment.
 *
 * Reversible: store the previous value in a backup table before updating.
 */
export class BackfillLegacyCaseStudentUuid20260702153000 implements MigrationInterface {
  name = 'BackfillLegacyCaseStudentUuid20260702153000';
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TABLE IF EXISTS case_student_uuid_backfill_20260702_backup`);
  }
}
