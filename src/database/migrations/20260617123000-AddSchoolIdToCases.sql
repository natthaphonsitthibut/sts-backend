-- Migration: AddSchoolIdToCases20260617123000
-- Adds a server-owned school scope to cases and backfills existing rows.
-- Primary backfill is cases.student_id -> latest student_term."SchoolID_Onec";
-- legacy fallback is the latest tasks.target_school_id attached to the case.
...
-- Last-resort fallback for legacy cases linked to a school only by its name text.
UPDATE cases c
...
-- == down ==
-- DROP INDEX IF EXISTS idx_cases_school_id;
-- ALTER TABLE cases
