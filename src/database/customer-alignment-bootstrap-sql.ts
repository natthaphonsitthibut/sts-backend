...
export const CUSTOMER_ALIGNMENT_FEATURE_TABLES_SQL = `
  ...
  CREATE TABLE IF NOT EXISTS classroom_additional_homeroom_teachers (
    classroom_id BIGINT NOT NULL,
    school_id INTEGER NOT NULL,
    teacher_membership_id BIGINT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  ...
  CREATE INDEX IF NOT EXISTS idx_classroom_additional_homeroom_teachers_membership
    ON classroom_additional_homeroom_teachers (teacher_membership_id, classroom_id);
  CREATE INDEX IF NOT EXISTS idx_classroom_additional_homeroom_teachers_created_by
    ON classroom_additional_homeroom_teachers (created_by)
    WHERE created_by IS NOT NULL;
  CREATE INDEX IF NOT EXISTS idx_classroom_additional_homeroom_teachers_updated_by
    ON classroom_additional_homeroom_teachers (updated_by)
    WHERE updated_by IS NOT NULL;
  DROP TRIGGER IF EXISTS trg_classroom_additional_homeroom_teachers_set_updated_at
  ...
  DROP TRIGGER IF EXISTS trg_prevent_duplicate_additional_homeroom_teacher
    ON classroom_additional_homeroom_teachers;
  ...
  DROP TRIGGER IF EXISTS trg_remove_promoted_additional_homeroom_teacher
    ON classroom_homeroom_teachers;
  ...
  DROP TRIGGER IF EXISTS trg_promote_additional_homeroom_teacher_after_primary_delete
    ON classroom_homeroom_teachers;
  ...
  BEGIN
    ...
    LOOP
      IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = role_name) THEN
        EXECUTE format(
          'REVOKE ALL PRIVILEGES ON TABLE teacher_external_identities, classroom_homeroom_teachers, classroom_additional_homeroom_teachers, classroom_homeroom_teacher_assignments, classroom_attendance_links FROM %I',
          role_name
        );
        EXECUTE format(
