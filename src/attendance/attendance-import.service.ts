...
import {
  ...
  type RecordAttendanceImportInput,
} from './attendance-import-history.repository';
...
export class AttendanceImportService {
  constructor(
    ...
    private readonly history: AttendanceImportHistoryRepository,
   ...
   */
  async recordApplied(
    input: Omit<RecordAttendanceImportInput, 'storageKey' | 'fileSizeBytes'> & {
    ...
    if (input.file?.buffer?.length) {
      if (input.file.buffer.byteLength > this.config.maxFileBytes) {
        throw new BadRequestException(this.fileTooLargeMessage());
      }
      const extension = extname(input.file.originalname) || '.xlsx';
      storageKey = `attendance-imports/${randomBytes(16).toString('hex')}${extension}`;
  ...
  async listApplied(input: {
    ...
    const rows = await this.history.list(input);
    return {
      rows: rows.map((row) => ({
        id: row.id,
  ...
  /** Streams a stored import back, scoped to the classroom that owns it. */
  async openApplied(id: number, classroomId: number) {
    const row = await this.history.findForDownload(id, classroomId);
    if (!row) throw new NotFoundException('ไม่พบไฟล์นำเข้าที่ต้องการ');
    if (!row.storage_key) {
      throw new NotFoundException('การนำเข้านี้มาจากลิงก์ ไม่มีไฟล์ให้ดาวน์โหลด');
    }
    const stream = await this.storage.open(row.storage_key);
    if (!stream) throw new NotFoundException('ไม่พบไฟล์นำเข้าที่ต้องการ');
    return { stream, fileName: row.file_name };
  }
