...
async function main() {
  ...
  try {
    ...
    // The back button belongs on the right of its row, the way it sits on every
    // page inside the app — a link page hides its breadcrumb, and that must not
    // let the button slide to the left.
    const backAlignment = await chrome.evaluate(
      `(() => {
        const back = [...document.querySelectorAll('a,button')].find(
          (node) => (node.innerText || '').trim() === 'ย้อนกลับ',
        );
        ...
        return JSON.stringify({
          gapRight: Math.round(rowBox.right - backBox.right),
          gapLeft: Math.round(backBox.left - rowBox.left),
        });
      })()`,
    );
    assert(
      backAlignment !== 'missing' && backAlignment !== 'no-row',
      `could not measure the back button alignment: ${backAlignment}`,
    );
    const alignment = JSON.parse(String(backAlignment));
    // The row above needs its own breathing room; hiding the breadcrumb must not
    // take the gap with it and leave the two buttons stacked against each other.
    const rowGap = Number(
      await chrome.evaluate(
        `(() => {
          const pick = (label) =>
          ...
          if (!back || !openCase) return -1;
          return Math.round(
            openCase.getBoundingClientRect().top - back.getBoundingClientRect().bottom,
          );
        })()`,
      ),
    );
    assert(
      rowGap >= 8,
      `the back button and the action below it are stacked too tightly: ${rowGap}px`,
    );
    assert(
      alignment.gapRight < alignment.gapLeft,
      `the back button is not on the right of its row: ${backAlignment}`,
    );
